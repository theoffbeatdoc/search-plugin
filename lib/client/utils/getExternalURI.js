...
function isBaseOverlapping(baseParts, routeParts) {
    const base = baseParts.join("");
    const route = routeParts.join("");
    return route.startsWith(base);
}
export function getExternalURI(documentPath, baseURI) {
    // Sanitize input
    ...
    for (let baseIdx = 0; baseIdx < baseParts.length && externalBaseURI === baseWithoutProtocol; baseIdx++) {
